# ╔═══════════════════════════════════════════════════════════════════╗
# ║  股市小白分析系統 Pro  V4.0  ─  全週期量化 × 頂級英文 Prompt 重構版 ║
...
def fetch_data(symbol: str, period: str):
    try:
        ...
        except: info = {'longName': symbol, 'sector': '其他'}
        return h, info, None
    except Exception as e: return None, None, f"抓取失敗: {str(e)[:50]}"
        ...
        for name, sym in syms.items():
            try:
                ...
                if not df.empty and len(df)>=2:
                    c1 = float(df["Close"].iloc[-2]); c2 = float(df["Close"].iloc[-1])
                    rows.append({"名稱":name, "現值":c2, "漲跌%":round((c2 - c1) / max(c1, 0.01) * 100, 2)})
            except: continue
    except: pass
    ...
    try:
        ...
        for sym in symbols:
            try:
                ...
                if not df.empty and len(df)>=2:
                    c1 = float(df["Close"].iloc[-2]); c2 = float(df["Close"].iloc[-1])
                    results[sym] = (c2, round((c2 - c1) / max(c1, 0.01) * 100, 2))
...
def calc_indicators(hist: pd.DataFrame) -> dict:
    ...
    if ind["ma5"]>ind["ma20"]>ind["ma60"] and rv>55 and mv>0: ind["status"], ind["sc"], ind["status_desc"] = "強勢多頭 📈", "🔴", "均線多頭排列＋RSI偏強＋MACD正值，三重確認多頭。"
    elif ind["ma5"]<ind["ma20"]<ind["ma60"] and rv<45 and mv<0: ind["status"], ind["sc"], ind["status_desc"] = "強勢空頭 📉", "🟢", "均線空頭排列＋RSI偏弱＋MACD負值，三重確認空頭。"
    elif rv<=30 and kv<20: ind["status"], ind["sc"], ind["status_desc"] = "雙重超賣 🟡", "🟡", "RSI與Stochastic雙超賣，反彈機率升高，需量能確認。"
    elif rv>=70 and kv>80: ind["status"], ind["sc"], ind["status_desc"] = "雙重超買 🟡", "🟡", "RSI與Stochastic雙超買，短線獲利了結壓力大。"
    elif abs(ind["ma5"]-ind["ma20"])/max(price,0.01)<0.015: ind["status"], ind["sc"], ind["status_desc"] = "均線糾結蓄勢 ⚪", "⚪", "均線纏繞，等待突破，大行情可能即將爆發。"
    elif ind["ma5"]>ind["ma20"] and rv>50: ind["status"], ind["sc"], ind["status_desc"] = "短線偏多 🔵", "🔵", "短均線在長均線上方，RSI偏強，短線多方略佔優勢。"
    else: ind["status"], ind["sc"], ind["status_desc"] = "弱勢盤整 🟠", "🟠", "走勢疲軟，方向不明，建議觀望。"
...
def run_dca(symbol: str, monthly_amount: float, years: int):
    ...
    stats={"總投入本金":round(total_inv,0),"最終資產現值":round(fv,0),"累積報酬率":f"{rtn:+.2f}%","年化報酬率":f"{rtn/max(years,1):+.2f}%","最大回撤MDD":f"{mdd:.2f}%","回測年限":f"{years}年","每月投入":f"{monthly_amount:,.0f}元"}
...
def call_ai(api_key: str, prompt: str, use_search: bool = False) -> str:
    ...
    for mid, mname in models:
        try:
            ...
            else: resp = client.models.generate_content(model=mid,contents=prompt)
            tag = "+Search" if use_search else ""; ts  = datetime.now().strftime("%Y-%m-%d %H:%M")
            return f"> [AI] {mname}{tag} | {ts}\n\n{resp.text}"
        except Exception as e:
            err=str(e)
            if any(k in err for k in ["quota","429","RESOURCE_EXHAUSTED","404","not found"]): last_err=err[:80]; continue
            raise
...
# 10. 圖表建構 (台灣色系: 紅漲綠跌)
# ══════════════════════════════════════════════
DARK = "plotly_dark"; TW_UP = "#ff3333"; TW_DOWN = "#22cc44"
...
def build_main_chart(hist, ind, entry, sym, mc_data=None, buy_markers=None):
    ...
    vc=[TW_UP if c>=o else TW_DOWN for c,o in zip(hist["Close"],hist["Open"])]; fig.add_trace(go.Bar(x=idx,y=ind["_v"],marker_color=vc,showlegend=False),row=2,col=1)
    ...
    mc_c=[TW_UP if x>=0 else TW_DOWN for x in ind["_macd_h"]]; fig.add_trace(go.Bar(x=idx,y=ind["_macd_h"],marker_color=mc_c,showlegend=False),row=4,col=1)
    fig.add_trace(go.Scatter(x=idx,y=ind["_macd"],line=dict(color="#42A5F5",width=1),showlegend=False),row=4,col=1); fig.add_trace(go.Scatter(x=idx,y=ind["_macd_sig"],line=dict(color="#FF7043",width=1),showlegend=False),row=4,col=1)
...
def build_portfolio_sunburst(portfolio: dict, sector_map: dict) -> go.Figure:
    rows = []
    for sym,data in portfolio.items(): rows.append({"板塊":sector_map.get(sym,"其他"),"代號":sym,"市值":data["cost"]*data["shares"]})
    if not rows: return None
    df = pd.DataFrame(rows); fig = px.sunburst(df,path=["板塊","代號"],values="市值",title="持股板塊旭日圖(方塊大小=持倉成本)",template=DARK,color_discrete_sequence=px.colors.qualitative.Set3)
    fig.update_layout(height=450,margin=dict(l=10,r=10,t=60,b=10))
    return fig

def build_dca_chart(df_dca: pd.DataFrame, sym: str) -> go.Figure:
    fig = go.Figure()
    fig.add_trace(go.Scatter(x=df_dca.index,y=df_dca["累積投入"],fill="tozeroy",name="累積投入本金",line=dict(color="#42A5F5",width=2),fillcolor="rgba(66,165,245,0.15)")); fig.add_trace(go.Scatter(x=df_dca.index,y=df_dca["資產現值"],fill="tozeroy",name="資產現值",line=dict(color=TW_UP,width=2),fillcolor="rgba(255,51,51,0.15)"))
    fig.update_layout(template=DARK,title=f"{sym} 定期定額回測",yaxis_title="金額(元)",height=420,legend=dict(orientation="h",y=1.02),margin=dict(l=50,r=30,t=60,b=30)); return fig

def build_mc_standalone(mc_data: dict, sym: str) -> go.Figure:
    fig = go.Figure(); fd = mc_data["dates"]; p = mc_data["pcts"]
    fig.add_trace(go.Scatter(x=fd,y=p["p97"],line=dict(color="rgba(255,200,50,0.3)",width=1),name="95%區間上界")); fig.add_trace(go.Scatter(x=fd,y=p["p02"],line=dict(color="rgba(255,200,50,0.3)",width=1),name="95%區間下界",fill="tonexty",fillcolor="rgba(255,200,50,0.08)")); fig.add_trace(go.Scatter(x=fd,y=p["p84"],line=dict(color="rgba(100,180,255,0.5)",width=1),name="68%區間上界")); fig.add_trace(go.Scatter(x=fd,y=p["p16"],line=dict(color="rgba(100,180,255,0.5)",width=1),name="68%區間下界",fill="tonexty",fillcolor="rgba(100,180,255,0.12)")); fig.add_trace(go.Scatter(x=fd,y=p["p50"],line=dict(color="#FFD700",width=2.5),name="中位數路徑"))
    for i,path in enumerate(mc_data["matrix_sample"][:20]): fig.add_trace(go.Scatter(x=fd,y=path,line=dict(color="rgba(200,200,200,0.07)",width=0.5),showlegend=False))
    fig.add_hline(y=mc_data["last"],line_dash="dash",line_color="rgba(255,255,255,0.5)",annotation_text="現價")
    fig.update_layout(template=DARK,title=f"{sym} 蒙地卡羅30日模擬",yaxis_title="預估價格",height=450,legend=dict(orientation="h",y=1.02),margin=dict(l=50,r=30,t=70,b=30)); return fig
...
with st.sidebar:
    ...
    if not st.session_state.logged_in:
        ...
        if st.button("🚪 登出", use_container_width=True):
            ...
            if cookie_controller is not None: cookie_controller.remove("tw_stock_u"); cookie_controller.remove("tw_stock_p")
            st.rerun()

    st.divider()
    if st.session_state.logged_in and st.session_state.api_key:
        st.success("🔑 API 金鑰已帶入")
        if st.button("🔄 更換金鑰"): st.session_state.api_key=""; st.rerun()
        ...
        if api_key_in and api_key_in!=st.session_state.api_key:
            ...
            if st.session_state.logged_in and HAS_DB: db_save_enc_key(st.session_state.username,st.session_state._pin,api_key_in); st.success("🔒 已加密")
        api_key = "".join(c for c in str(st.session_state.api_key) if ord(c) < 128).strip() if st.session_state.api_key else ""
        ...
        with st.spinner("搜尋中..."):
            try:
                ...
                for r in MACRO_REGIMES[1:]:
                    if r in regime_rpt: st.session_state.macro_regime=r; macro_regime=r; break
                st.success(f"✅ {st.session_state.macro_regime}")
                with st.expander("查看分析"): st.markdown(regime_rpt)
...
# 12. 主頁面 (戰情室)
# ══════════════════════════════════════════════
st.title("📈 股市小白分析系統 Pro V4")
st.caption("AI三方辯論 × 全週期整合(當沖/短線/長線) × 頂級英文 Prompt 推理能力")

mkt = fetch_market_overview()
if mkt:
    mcols = st.columns(len(mkt))
    for col,row in zip(mcols,mkt): chg=row["漲跌%"]; col.metric(row["名稱"],str(row["現值"]),f"{'▲' if chg>=0 else '▼'}{abs(chg):.2f}%",delta_color="inverse")

if st.session_state.recent_searches:
    st.markdown("**🕐 最近查詢: **")
    rc=st.columns(min(len(st.session_state.recent_searches),8))
    for i,rs in enumerate(st.session_state.recent_searches):
        if rc[i].button(rs,key=f"rc_{i}"): st.session_state.quick_sym=rs.replace(".TW",""); st.session_state.auto_analyze=True; st.rerun()
...
with TABS[0]:
    ...
    for cat,stocks in hot.items():
        ...
        for col,(sym_,name_) in zip(qcols,stocks):
            if col.button(f"{sym_}\n{name_}",use_container_width=True,key=f"qs_{sym_}_{cat}"): st.session_state.quick_sym=sym_; st.session_state.auto_analyze=True; st.rerun()
    ...
    elif st.session_state.auto_analyze and st.session_state.quick_sym: st.session_state.current_sym = st.session_state.quick_sym; st.session_state.auto_analyze = False
    ...
    if st.session_state.current_sym:
        ...
        with st.spinner(f"抓取 {sym} 數據..."): hist, info, err = fetch_data(sym, period)
        ...
        with kpi_col:
            ...
            r1,r2,r3 = st.columns(3); r1.metric("💰 現價", ind["price"]); r2.metric("今日", f"{ci}{ind['change_pct']}%", delta=str(ind["change_pct"]), delta_color="inverse"); r3.metric("本月", f"{ind['change_1m']}%", delta=str(ind["change_1m"]), delta_color="inverse")
        ...
        with st.expander("💰 詳細區間目標價參考 (不構成投資建議)", expanded=False):
            ...
            with e1: st.markdown(f'<div class="entry-card"><div style="color:#66cc66;font-weight:bold">🎯 買入區</div><div style="margin:8px 0;color:#e2e8f0;line-height:2">長線建倉: <b>{entry["con_buy"]}</b><br>短線波段: <b>{entry["mod_buy"]}</b><br>當沖突破: <b>{entry["agg_buy"]}</b></div></div>',unsafe_allow_html=True)
            with e2: st.markdown(f'<div class="stop-card"><div style="color:#ff6666;font-weight:bold">🛡️ 停損區</div><div style="margin:8px 0;color:#e2e8f0;line-height:2">當沖極短: <b>{entry["sl_tight"]}</b><br>波段標準: <b>{entry["sl_normal"]}</b><br>長線寬幅: <b>{entry["sl_wide"]}</b></div></div>',unsafe_allow_html=True)
            with e3: st.markdown(f'<div class="target-card"><div style="color:#66aaff;font-weight:bold">🎯 目標價</div><div style="margin:8px 0;color:#e2e8f0;line-height:2">T1(短): <b>{entry["tp1"]}</b><br>T2(中): <b>{entry["tp2"]}</b><br>T3(壓力): <b>{entry["tp3"]}</b></div></div>',unsafe_allow_html=True)
        ...
        if sym in st.session_state.portfolio:
            ...
            for pd2 in entries:
                if pd2.get("buy_date"):
                    try:
                        bdate = pd.to_datetime(pd2["buy_date"]); if bdate.tz: bdate=bdate.tz_localize(None)
        ...
        if not api_key: st.warning("⚠️ 請先於左側輸入 Gemini API 金鑰啟用 AI 分析。")
        elif st.button("⚔️ 啟動三方辯論(約90秒)",type="primary"):
            with st.spinner("AI 英文底層邏輯推理中 (輸出將轉為繁體中文)..."):
                try:
                    ...
                    with debate_tab3:
                        ...
                        for sec in secs[1:]:
                            lines=sec.split("\n",1)
                            if "裁決" in lines[0] or "預測" in lines[0] or "建議" in lines[0] or "Verdict" in lines[0] or "Plan" in lines[0]:
...
with TABS[1]:
    ...
    with asset_tab1:
        ...
        with p5: st.write(""); st.write(""); p_btn=st.button("📊 計算",use_container_width=True,type="primary",key="pf_calc")
            ...
            if pe_: st.error(f"❌ {pe_}")
                ...
                if api_key:
                    if st.button("[AI] 多週期覆盤教練",key="review_btn",type="primary"):
                        ...
                        if _h2 is not None:
                            try:
                                _h2_idx=_h2.copy(); _h2_idx.index=pd.to_datetime(_h2_idx.index); if _h2_idx.index.tz: _h2_idx.index=_h2_idx.index.tz_localize(None)
                                near=_h2_idx.index[_h2_idx.index.searchsorted(pd.to_datetime(str(pd_)))]; row_buy=_h2_idx.loc[near]
                                ind_at_buy_str=f"收盤: {row_buy['Close']:.2f}"
                            except: ind_at_buy_str="無數據"
                ...
                with c2_s:
                    if ps_sym in st.session_state.portfolio:
                        if st.button("🗑️ 移除此檔",key="del_pf"):
                            db_del_port(st.session_state.user_id,ps_sym); del st.session_state.portfolio[ps_sym]
                            st.session_state.show_pf_calc = False; st.rerun()

        if st.session_state.portfolio:
            st.divider(); st.markdown("### 📋 持股組合總覽")
            sector_map = {}; pr_rows = []; tc_all = 0; cv_all = 0
            
            for sym, entries in st.session_state.portfolio.items():
                if isinstance(entries, dict): entries = [entries]
                total_s = sum(e.get("shares", 0) for e in entries); total_c = sum(e.get("cost", 0) * e.get("shares", 0) for e in entries)
                avg_c = round(total_c / total_s, 2) if total_s > 0 else 0
                try:
                    hh2 = yf.Ticker(sym).history(period="2d")
                    if not hh2.empty:
                        cp2 = round(float(hh2["Close"].iloc[-1]), 2); pnl_pct = round((cp2 - avg_c) / max(avg_c, 0.01) * 100, 2)
            ...
            with del_c1: target_sym = st.selectbox("1. 選擇股票", options=list(st.session_state.portfolio.keys()))
            with del_c2:
                if target_sym:
    ...
    with asset_tab3:
        ...
        with st.expander("➕ 新增交易"):
            ...
            with t5: t_ed=st.date_input("買入日",key="th_ed"); t_xd=st.date_input("賣出日",key="th_xd"); t_note=st.text_input("備注",key="th_note")
            if st.button("💾 記錄此交易",key="add_trade_btn"):
                if t_sym.strip() and t_ep>0 and t_xp>0 and t_sh>0:
                    ts_sym=get_sym(t_sym.strip(),market); pnl_a=round((t_xp-t_ep)*t_sh,2); pnl_p=round((t_xp-t_ep)/max(t_ep,0.01)*100,2)
...
with TABS[2]:
    ...
    with scr_tab:
        ...
        with col1: scr_rsi_lt = st.number_input("RSI <",0.0,100.0,35.0,step=5.0); scr_macd = st.checkbox("MACD > 0"); scr_ma20 = st.checkbox("站上月線")
        with col2: scr_rsi_gt = st.number_input("RSI >",0.0,100.0,0.0,step=5.0); scr_vol = st.checkbox("放量>1.5"); scr_bb_lo = st.checkbox("布林<25%")
        if st.button("🔍 掃描熱門股",type="primary",use_container_width=True):
            ...
            cond={}; if scr_rsi_lt>0: cond["rsi_lt"]=scr_rsi_lt; if scr_rsi_gt>0: cond["rsi_gt"]=scr_rsi_gt
            if scr_macd: cond["macd_cross_up"]=True; if scr_ma20: cond["above_ma20"]=True
            if scr_vol: cond["vol_spike"]=True; if scr_bb_lo: cond["bb_near_lower"]=True
    ...
    with dca_tab:
        ...
        if dca_btn and dca_sym.strip():
            with st.spinner("模擬中..."): df_dca,stats,err=run_dca(get_sym(dca_sym.strip(),market),dca_amt,dca_yr)
            if df_dca is None: st.error(err)
            else:
                for col,(k,v) in zip(st.columns(len(stats)),stats.items()): col.metric(k,v)
...
with TABS[4]:
    ...
    if len(trades)<3: st.warning("⚠️ 需要至少3筆已平倉記錄才能進行行為分析。")
        ...
        if bias:
            b1,b2,b3,b4 = st.columns(4); b1.metric("勝率",f"{bias['win_rate']}%"); b2.metric("平均獲利",f"+{bias['avg_win_pct']:.2f}%"); b3.metric("平均虧損",f"-{bias['avg_loss_pct']:.2f}%"); b4.metric("盈虧比",f"{round(bias['avg_win_pct']/max(bias['avg_loss_pct'],0.01),2)}x")
            b5,b6,b7 = st.columns(3); b5.metric("持有獲利天數",f"{bias['avg_win_days']:.0f}天"); b6.metric("持有虧損天數",f"{bias['avg_loss_days']:.0f}天",delta="⚠️ 異常" if bias['avg_loss_days']>bias['avg_win_days'] else "正常",delta_color="off")
...
st.divider(); st.caption("📈 股市小白分析系統 Pro V4.0 | 🔴紅漲🟢跌 | ⚠️ 內容僅供學習參考，不構成投資建議")