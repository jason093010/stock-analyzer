...
# ╚═══════════════════════════════════════════════════════════════════╝
import sys, io
# 💡 終極補丁: 強制設定系統輸出為 UTF-8，解決 Windows ASCII 編碼地雷
try:
...
GLOSSARY = {
    "RSI":"相對強弱指標0~100。>70超買，<30超賣。",
    "MACD":"動能指標。柱狀圖正值=動能增強，負值=動能減弱。",
    "布林通道":"股價正常波動範圍。通道收窄=行情即將爆發。",
    "ATR":"每天平均波動多少錢。",
    ...
    "MDD最大回撤":"從最高點到最低點的最大跌幅。",
    "蒙地卡羅":"用歷史波動率隨機模擬未來可能路徑。",
    "VIX恐慌指數":">30=市場極度恐慌，<15=市場過度樂觀。",
    "處置效應":"散戶常見心理偏誤: 太早賣出獲利股，太晚出清虧損股。",
    "總體宏觀制度":"指當前全球經濟所處的大環境。",
}
...
def calc_indicators(hist: pd.DataFrame) -> dict:
    ...
    if ind["ma5"]>ind["ma20"]>ind["ma60"] and rv>55 and mv>0:
        ind["status"]="強勢多頭";ind["sc"]="🔴";ind["status_desc"]="均線多頭排列＋RSI偏強＋MACD正值"
    elif ind["ma5"]<ind["ma20"]<ind["ma60"] and rv<45 and mv<0:
        ind["status"]="強勢空頭";ind["sc"]="🟢";ind["status_desc"]="均線空頭排列＋RSI偏弱＋MACD負值"
    elif rv<=30 and kv<20:
        ind["status"]="雙重超賣";ind["sc"]="🟡";ind["status_desc"]="RSI與Stochastic雙超賣"
    elif rv>=70 and kv>80:
        ind["status"]="雙重超買";ind["sc"]="🟡";ind["status_desc"]="RSI與Stochastic雙超買"
    elif abs(ind["ma5"]-ind["ma20"])/max(price,0.01)<0.015:
        ind["status"]="均線糾結";ind["sc"]="⚪";ind["status_desc"]="均線纏繞，等待突破"
    elif ind["ma5"]>ind["ma20"] and rv>50:
        ind["status"]="短線偏多";ind["sc"]="🔵";ind["status_desc"]="短均線在長均線上方"
    else:
        ind["status"]="弱勢盤整";ind["sc"]="🟠";ind["status_desc"]="走勢疲軟，方向不明"

    if rv>=80:    ind["rsi_desc"]=f"RSI {rv}，嚴重超買"
    elif rv>=70:  ind["rsi_desc"]=f"RSI {rv}，超買區"
    elif rv<=20:  ind["rsi_desc"]=f"RSI {rv}，嚴重超賣"
    elif rv<=30:  ind["rsi_desc"]=f"RSI {rv}，超賣區"
    elif rv>=55:  ind["rsi_desc"]=f"RSI {rv}，多方偏強"
    elif rv<=45:  ind["rsi_desc"]=f"RSI {rv}，空方偏強"
    else:         ind["rsi_desc"]=f"RSI {rv}，多空均衡"
    return ind
...
def run_screener(symbols: list, conditions: dict) -> list:
    ...
    def _check(sym):
        try:
            ...
            if ok:
                return {"代號":sym,"現價":ind["price"],"RSI":ind["rsi"],"MACD_H":ind["macd_hist"],
...
def call_ai(api_key: str, prompt: str, use_search: bool = False) -> str:
    ...
    models = [("gemini-2.0-flash","Gemini 2.0 Flash"), ("gemini-2.0-flash-lite","Gemini Flash Lite")]
    last_err = None
...
{resp.text}"
            ...
            raise
    raise Exception(f"AI Failed: {last_err}")

def ai_macro_regime(api_key: str) -> str:
    prompt = "請即時搜尋並判斷當前全球總體經濟所處的宏觀制度(Macro Regime)。從以下選項選擇最符合的一個並說明理由: 成長擴張, 通膨衰退, 衰退, 復甦反彈, 流動性危機。格式: **制度:** [選項名稱] **理由:** [說明]。繁體中文。"
...
def ai_three_agent_debate(ind, info, sym, api_key, entry, score, macro_regime) -> tuple:
    ...
    base_data = f"股票: {co} ({sym}) | 現價: {ind['price']} | 今日: {ind['change_pct']}% | 月: {ind['change_1m']}%
評分: {score['total']}/100 | 狀態: {ind['status']}
MA5={ind['ma5']} MA20={ind['ma20']} MA60={ind['ma60']}
RSI={ind['rsi']} MACD_H={ind['macd_hist']} ATR={atr}
布林位置={ind['bb_pct']:.0f}%
當前宏觀制度: {macro_regime}"

    bull_prompt = f"你是一位狂熱的多頭分析師。你的任務是找出所有看漲理由。
{base_data}
請即時搜尋最新正面消息後，輸出(條列式,繁體中文):
## 多頭論點
- 核心催化劑
- 技術面多頭證據
- ATR多頭情境: 目標 {bull_range}
- 多頭信心評級: ⬛/5"
    bear_prompt = f"你是一位極度悲觀的空頭分析師。你的任務是無情揭露所有看跌風險。
{base_data}
請即時搜尋最新負面消息後，輸出(條列式,繁體中文):
## 空頭論點
- 核心威脅
- 技術面空頭證據
- ATR空頭情境: 目標 {bear_range}
- 空頭信心評級: ⬛/5"
    judge_prompt = f"你是一位冷靜理性的投資長。你已收到多空辯論，請做出最終裁決。
{base_data}
輸出(條列式,繁體中文):
## CIO最終裁決
- 多空力道對比
...
def ai_portfolio_cio(portfolio_data: str, sector_data: str, api_key: str) -> str:
    prompt = f"你是一位投資長(CIO)，負責審查以下持股組合並給予整併建議。
{portfolio_data}
板塊分佈: {sector_data}
請即時搜尋各持股動態後輸出(繁體中文):
## 組合健康診斷
...
def ai_bias_warning(bias_data: dict, trades_summary: str, api_key: str) -> str:
    ...
    prompt = f"你是行為財務學專家，分析以下交易記錄。
勝率: {bias_data.get('win_rate',0)}%
平均獲利: +{bias_data.get('avg_win_pct',0):.2f}%
平均虧損: -{bias_data.get('avg_loss_pct',0):.2f}%
...
def build_main_chart(hist, ind, entry, sym, mc_data=None, buy_markers=None):
    ...
    titles = ["K線+均線+布林","成交量","RSI+Stochastic","MACD"]
    if mc_data:
        rows=5; heights=[0.42,0.18,0.15,0.13,0.12]
        titles.append("蒙地卡羅模擬")

    fig = make_subplots(rows=rows,cols=1,shared_xaxes=True,row_heights=heights,subplot_titles=titles,vertical_spacing=0.03)

    fig.add_trace(go.Candlestick(x=idx,open=hist["Open"],high=hist["High"],low=hist["Low"],close=hist["Close"],name="K線",
        increasing_line_color=TW_UP, decreasing_line_color=TW_DOWN, increasing_fillcolor=TW_UP, decreasing_fillcolor=TW_DOWN),row=1,col=1)
    ...
    for y_val,color,label in [(entry["sup1"],"rgba(34,204,68,0.8)",f"支撐 {entry['sup1']}"),(entry["res1"],"rgba(255,51,51,0.8)", f"壓力 {entry['res1']}"),(entry["mod_buy"],"rgba(255,200,50,0.8)",f"買點 {entry['mod_buy']}")]:
...
{bm['price']}"],textposition="bottom center",textfont=dict(color="#FFD700",size=10),name=f"買入 {bm['price']}",showlegend=True),row=1,col=1)
    ...
    if mc_data:
        ...
        fig.add_trace(go.Scatter(x=fd,y=p["p97"],line=dict(color="rgba(255,200,50,0.3)",width=1),showlegend=False,name="95%"),row=mc_row,col=1)
        ...
        fig.add_trace(go.Scatter(x=fd,y=p["p84"],line=dict(color="rgba(100,180,255,0.5)",width=1),showlegend=False,name="68%"),row=mc_row,col=1)
    ...
    fig.update_layout(template=DARK,title=f"{sym} 技術分析",height=900 if mc_data else 780,xaxis_rangeslider_visible=False,legend=dict(orientation="h",y=1.02,font_size=11),margin=dict(l=50,r=50,t=80,b=30))
    return fig

def build_heatmap_chart(df: pd.DataFrame, market: str):
    if df.empty: return None
    fig = px.treemap(df,path=["板塊","名稱"],values="市值",color="漲跌%",color_continuous_scale=[(0.0,TW_DOWN),(0.5,"#333333"),(1.0,TW_UP)],color_continuous_midpoint=0,custom_data=["代號","漲跌%"],title=f"{market} 板塊熱力圖",template=DARK)
...
def build_portfolio_sunburst(portfolio: dict, sector_map: dict) -> go.Figure:
    ...
    fig = px.sunburst(df,path=["板塊","代號"],values="市值",title="持股板塊旭日圖",template=DARK,color_discrete_sequence=px.colors.qualitative.Set3)
...
def build_dca_chart(df_dca: pd.DataFrame, sym: str) -> go.Figure:
    ...
    fig.update_layout(template=DARK,title=f"{sym} 定期定額回測",yaxis_title="金額",height=420,legend=dict(orientation="h",y=1.02),margin=dict(l=50,r=30,t=60,b=30))
    return fig

def build_mc_standalone(mc_data: dict, sym: str) -> go.Figure:
    fig = go.Figure()
    fd = mc_data["dates"]; p = mc_data["pcts"]
    fig.add_trace(go.Scatter(x=fd,y=p["p97"],line=dict(color="rgba(255,200,50,0.3)",width=1),name="95%上界"))
    fig.add_trace(go.Scatter(x=fd,y=p["p02"],line=dict(color="rgba(255,200,50,0.3)",width=1),name="95%下界",fill="tonexty",fillcolor="rgba(255,200,50,0.08)"))
    fig.add_trace(go.Scatter(x=fd,y=p["p84"],line=dict(color="rgba(100,180,255,0.5)",width=1),name="68%上界"))
    fig.add_trace(go.Scatter(x=fd,y=p["p16"],line=dict(color="rgba(100,180,255,0.5)",width=1),name="68%下界",fill="tonexty",fillcolor="rgba(100,180,255,0.12)"))
    fig.add_trace(go.Scatter(x=fd,y=p["p50"],line=dict(color="#FFD700",width=2.5),name="中位數"))
...
with st.sidebar:
    ...
    if not st.session_state.logged_in:
        with st.expander("🔐 登入 / 建立帳號", expanded=True):
            ...
            ca, cb = st.columns(2)
            if ca.button("🔑 登入", use_container_width=True):
                if uname.strip() and upin.strip():
                    ...
                    if ok:
                        ...
                        if enc: dec = decrypt_key(enc, upin.strip()); st.session_state.api_key = dec if dec else ""
                        st.success("登入成功"); st.rerun()
                    else: st.error("帳密錯誤")
            if cb.button("✨ 建立", use_container_width=True):
                if uname.strip() and upin.strip():
    ...
    if st.session_state.logged_in and st.session_state.api_key:
        ...
        api_key_in = st.text_input("🔑 Gemini API 金鑰",type="password")
    ...
    macro_regime = st.selectbox("🌍 宏觀制度",MACRO_REGIMES,index=MACRO_REGIMES.index(st.session_state.macro_regime))
    ...
    st.divider()
    st.header("⭐ 自選股")
    nw = st.text_input("新增代號",key="sb_nw")
    if nw and nw.strip():
    ...
    st.divider()
    st.header("🔔 警報")
    al_s=st.text_input("代號",key="al_s")
    al_u=st.number_input("↑漲破",0.0,step=0.5)
    al_d=st.number_input("↓跌破",0.0,step=0.5)
...
{name_}",use_container_width=True,key=f"qs_{sym_}_{cat}"):
    ...
    with ic2: st.write(""); go_btn = st.button("🔍 開始分析",use_container_width=True,type="primary")
    ...
    if should_run and (ticker_in or st.session_state.quick_sym).strip():
        ...
        if not api_key: st.warning("⚠️ 請先輸入 API 金鑰"); st.stop()
        sym = get_sym(use_sym, market); add_recent(sym)

        with st.spinner(f"抓取 {sym} ..."): hist, info, err = fetch_data(sym, period)
        if err: st.error(err); st.stop()

        ind   = calc_indicators(hist); entry = calc_entry(ind, hist); score = calc_score(ind, info)
        co    = info.get("longName") or info.get("shortName") or sym
        ts    = datetime.now().strftime("%Y-%m-%d %H:%M")
        ...
        with score_col:
            st.markdown(f'<div class="score-card"><div class="big-score" style="color:{score["gc"]}">{score["total"]}</div></div>',unsafe_allow_html=True)
            st.progress(score["trend"]/30,text=f"趨勢 {score['trend']}/30"); st.progress(score["mom"]/25,text=f"動能 {score['mom']}/25")
            st.progress(score["vol"]/20,text=f"量能 {score['vol']}/20"); st.progress(score["pos"]/15,text=f"位置 {score['pos']}/15")
        with kpi_col:
            st.markdown(f'<div class="status-card"><h3 style="margin:0;color:#e2e8f0">{ind["sc"]} {ind["status"]}</h3></div>',unsafe_allow_html=True)
            r1,r2,r3 = st.columns(3)
            r1.metric("💰 現價",ind["price"]); r2.metric("今日",f"{ind['change_pct']}%",delta=str(ind["change_pct"]),delta_color="inverse"); r3.metric("本月",f"{ind['change_1m']}%",delta=str(ind['change_1m']),delta_color="inverse")

        with st.expander("💰 入場策略參考"):
            e1,e2,e3 = st.columns(3)
            with e1: st.markdown(f'<div class="entry-card">穩健: <b>{entry["mod_buy"]}</b></div>',unsafe_allow_html=True)
            with e2: st.markdown(f'<div class="stop-card">標準: <b>{entry["sl_normal"]}</b></div>',unsafe_allow_html=True)
            with e3: st.markdown(f'<div class="target-card">T1: <b>{entry["tp1"]}</b></div>',unsafe_allow_html=True)

        st.markdown("### 📈 技術分析圖表")
        show_mc = st.checkbox("🎲 蒙地卡羅模擬")
        mc_data = monte_carlo_simulation(hist) if show_mc else None
        
        buy_markers = []
        if sym in st.session_state.portfolio:
            for bm_entry in (st.session_state.portfolio[sym] if isinstance(st.session_state.portfolio[sym], list) else [st.session_state.portfolio[sym]]):
                if bm_entry.get("buy_date"):
                    try: buy_markers.append({"date":pd.to_datetime(bm_entry["buy_date"]),"price":bm_entry["cost"]})
                    except: pass
                    
        st.plotly_chart(build_main_chart(hist,ind,entry,sym,mc_data,buy_markers),use_container_width=True)

        st.markdown("### 🤖 AI 三方辯論分析")
        if st.button("⚔️ 啟動三方辯論",type="primary"):
...
with TABS[1]:
    ...
    with asset_tab1:
        ...
        with p5:
            st.write(""); p_btn=st.button("📊 計算",use_container_width=True,type="primary")
        ...
        if st.session_state.show_pf_calc and ps_.strip() and pc_>0 and pn_>0:
            ...
            ph_,_,pe_=fetch_data(ps_sym,"5d")
            if pe_: st.error(pe_)
            else:
                cp_=round(float(ph_["Close"].iloc[-1]),2)
                pnl=round((cp_-pc_)*pn_,2); pct_v=round((cp_-pc_)/max(pc_,0.01)*100,2)
                st.metric(f"{ps_sym} 當前損益", f"{pnl:+,.0f}", delta=f"{pct_v:+.2f}%", delta_color="inverse")
                if st.button("💾 存入持股記錄"):
                    db_save_port(st.session_state.user_id, ps_sym, pc_, pn_, "", str(pd_))
                    st.session_state.portfolio = db_load_port(st.session_state.user_id)
                    st.success("✅ 已存入")
                    st.rerun()
        ...
        if st.session_state.portfolio:
            ...
            for sym, entries in st.session_state.portfolio.items():
                ...
                try:
                    ...
                    sector_map[sym] = sec
                    pr_rows.append({"代號": sym, "均價": avg_c, "現價": cp2, "損益%": f"{pnl_pct:+.2f}%", "總股數": total_s, "板塊": sec})
                    tc_all += c2; cv_all += v2
                except: pass
            ...
            if pr_rows:
                ...
                if api_key and st.button("🏦 AI投資長 (CIO) 組合審查", type="primary"):
                    port_str = "\n".join([f"- {r['代號']}: 均價 {r['均價']} 現價 {r['現價']} 損益 {r['損益%']} 板塊 {r.get('板塊','未知')}" for r in pr_rows])
                    sec_count = {r.get("板塊","其他"):1 for r in pr_rows}
                    with st.spinner("AI審查中..."):
                        try:
            ...
            with del_c2:
                if target_sym:
                    ...
                    selected_label = st.selectbox("2. 選擇筆數", options=list(entry_options.keys()))
    ...
    with asset_tab2:
        ...
        if st.session_state.watchlist:
            ...
            wrows=[{"代號":s,"現價":quotes.get(s,(None,None))[0],"今日":f"{quotes.get(s,(None,None))[1]}%"} for s in st.session_state.watchlist]
    ...
    with asset_tab3:
        ...
        if trades:
            st.dataframe(pd.DataFrame([{"代號":t.get("symbol",""),"損益":f"{t.get('pnl_amount',0):+,.0f}","損益%":f"{t.get('pnl_pct',0):+.2f}%"} for t in trades]))

with TABS[2]:
    scr_tab, dca_tab = st.tabs(["🔎 技術面選股器","⏳ 定期定額回測"])
    with dca_tab:
        d1,d2,d3,d4=st.columns(4)
        with d1: dca_sym=st.text_input("代號",key="dca_s")
        with d2: dca_amt=st.number_input("每月投入",1000.0,1000000.0,10000.0,step=1000.0)
        with d3: dca_yr =st.selectbox("年限",[3,5,10,15,20],index=1)
        with d4:
            st.write(""); dca_btn=st.button("⏳ 回測",type="primary",use_container_width=True)
        if dca_btn and dca_sym:
            df_dca,stats,err=run_dca(get_sym(dca_sym,market),dca_amt,dca_yr)
...
with TABS[3]:
    st.markdown("### 🗺️ 板塊熱力圖")
    if st.button("🔄 載入熱力圖",type="primary"):
...
with TABS[4]:
    ...
    trades=st.session_state.trade_history
    if len(trades)<3:
        st.warning("⚠️ 需要至少3筆已平倉交易記錄才能分析。")
    else:
        bias=analyze_behavioral_bias(trades)
        b1,b2,b3,b4 = st.columns(4)
        b1.metric("勝率",f"{bias.get('win_rate',0)}%")
        b2.metric("平均獲利",f"+{bias.get('avg_win_pct',0):.2f}%")
        b3.metric("平均虧損",f"-{bias.get('avg_loss_pct',0):.2f}%")
        if api_key and st.button("🧠 AI 深度行為偏誤診斷",type="primary"):
            summary="\n".join([f"- {t.get('symbol','')}: 損益{t.get('pnl_pct',0):+.1f}%" for t in trades[:10]])
            with st.spinner("AI分析中..."):
                st.markdown(ai_bias_warning(bias,summary,api_key))
